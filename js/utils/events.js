...
var events_addHandler = function (dom, type, handler) {
    function wrapHandler(event) {
        handler(handleBrowserDiffs(event || window.event));
    }
    registerHandler(dom, type, wrapHandler);
...
var events_removeHandler = function (object) {
    if ( object != null ) {
        unregisterHandler(object.dom, object.type, object.handler);
    }
};

export { events_addHandler as addHandler, events_removeHandler as removeHandler };