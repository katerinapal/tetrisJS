...
var testsDiv = utilsdomjs_create( "div", {
    style : 'overflow : auto;'
} );
...
testsDiv.appendChild( utilsdomjs_create( "div", {
    style : " height : 500px; placement : absolute "
} ) );

function prepATest( pDom, test ) {

    test.status = utilsdomjs_create( "string", "Waiting" );
    test.passFail = utilsdomjs_create( "div", null,
            test.status );
    test.msgDiv = utilsdomjs_create(
                    "div",
                    {
    ...
    // the text.
    utilseventsjs_addHandler( test.passFail, "mouseenter",
            function() {
                test.msgDiv.style.display = "block";
            } );
    utilseventsjs_addHandler( test.passFail, "mouseleave",
            function() {
                test.msgDiv.style.display = "none";
            } );

    var board = new Board( test.board );
    var runit = utilsdomjs_create( "BUTTON", null, "Run" );
    var div = utilsdomjs_create( "div", {
        style : 'float : left; margin-left : 10px;'
    }, utilsdomjs_create( "p", null, test.title ), runit,
    ...
    utilseventsjs_addHandler( runit, "click", function() {
...
function doAction( id, board, test, msgdiv ) {
    ...
    if ( board.shape.version === test.version
        ...
        msgdiv.appendChild( utilsdomjs_create( "p", null,
                "Test " + id + " : passed " ) );
        return true;
    }
    msgdiv.appendChild( utilsdomjs_create( "p", null, "Test "
            + id + " : failed " + ": version - "
