...
testsDiv.appendChild( dom_create( "div", {
    style : " height : 500px; placement : absolute "
} ) );

function prepATest( pDom, test ) {

    test.status = dom_create( "string", "Waiting" );
    test.passFail = dom_create( "div", null,
            test.status );
    test.msgDiv = dom_create(
                    "div",
                    {
    ...
    // the text.
    events_addHandler( test.passFail, "mouseenter",
            function() {
                test.msgDiv.style.display = "block";
            } );
    events_addHandler( test.passFail, "mouseleave",
            function() {
                test.msgDiv.style.display = "none";
            } );

    var board = new Board( test.board );
    var runit = dom_create( "BUTTON", null, "Run" );
    var div = dom_create( "div", {
        style : 'float : left; margin-left : 10px;'
    }, dom_create( "p", null, test.title ), runit,
    ...
    events_addHandler( runit, "click", function() {
...
function doAction( id, board, test, msgdiv ) {
    ...
    if ( board.shape.version === test.version
        ...
        msgdiv.appendChild( dom_create( "p", null,
                "Test " + id + " : passed " ) );
        return true;
    }
    msgdiv.appendChild( dom_create( "p", null, "Test "
            + id + " : failed " + ": version - "
